...
var passwordChangeOld = document.getElementById('old');
var passwordChangeNew = document.getElementById('new');
var gameSearch = document.getElementById('game-id');
...
var openStats = function openStats() {
  statsModal.style.display = 'block';
};
var closeStats = function closeStats() {
...
var incrementTurns = function incrementTurns() {
  turnsTaken = turnsTaken + 1;
  turn = turn + 1;
};
var determineCurrentPlayer = function determineCurrentPlayer() {
  if (turnsTaken % 2 === 0) {
    currentPlayer = 'X';
  } else {
    currentPlayer = 'O';
...
var getCellId = [document.getElementById('0'), document.getElementById('1'), document.getElementById('2'), document.getElementById('3'), document.getElementById('4'), document.getElementById('5'), document.getElementById('6'), document.getElementById('7'), document.getElementById('8')];
...
var newConfirm = document.getElementById('new-confirm');
...
var onSignIn = function onSignIn(event) {
  var data = getFormFields(this);
  event.preventDefault();
  api.signIn(data).then(ui.signInSuccess).catch(ui.signInFailure).then(onCreateGameSignIn);
};
...
var onCreateGameSignIn = function onCreateGameSignIn() {
  api.create().then(ui.onCreateSuccessSignIn).catch(ui.signInFailure);
};

var onCreateGame = function onCreateGame(event) {
  event.preventDefault();
  api.create().then(ui.onCreateSuccess).catch(ui.onError);
};
...
var onUpdateGame = function onUpdateGame(event) {
  ...
  var game = data.game;
  api.update(game).then(ui.onUpdateSuccess).catch(ui.onUpdateError);
};

var onGetGames = function onGetGames() {
  event.preventDefault();
  api.index().then(ui.onGetGamesSuccess).catch(ui.onErrorModal);
};
...
var onGetGame = function onGetGame(event) {
  ...
  var game = data.game;
  api.show(game.id).then(ui.onGetGameSuccess).catch(ui.onErrorModal);
};
...
var onCreateSuccessSignIn = function onCreateSuccessSignIn(data) {
...
var onErrorModal = function onErrorModal() {
...
module.exports = {
  ...
  onCreateSuccessSignIn: onCreateSuccessSignIn,
